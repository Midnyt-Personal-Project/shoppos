...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-3 gap-4">
        ...
        <div class="card p-5 ">
            <p class="text-slate-500 text-xs">Total Stock Value (at profit)</p>
            <p class="text-3xl font-bold text-white mt-1">₵{{ number_format($stockProfitValue, 2) }}</p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-800 text-slate-500 text-xs uppercase tracking-wider">
                    ...
                    <th class="text-right px-3 py-3">Selling Value</th>
                    <th class="text-right px-3 py-3">Profit Value</th>
            ...
            <tbody class="divide-y divide-slate-800">
                ...
                <tr class="hover:bg-white/[0.02]">
                    <td class="px-5 py-3">
                        ...
                        <p class="text-slate-400 text-xs bg-yellow-500/10 text-yellow-400">Cost: ₵{{ number_format($stock->product->cost, 2) }}</p>
                        <p class="text-slate-400 text-xs bg-green-500/10 text-green-400">Selling: ₵{{ number_format($stock->product->price, 2) }}</p>
                    </td>
                    ...
                    <td class="px-3 py-3 text-right text-slate-300">₵{{ number_format($stock->quantity * $stock->product->price, 2) }}</td>
                    <td class="px-3 py-3 text-right text-green-300">₵{{ number_format($stock->quantity * ($stock->product->price - $stock->product->cost), 2) }}</td>
