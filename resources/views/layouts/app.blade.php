...
<body class="h-full" x-data="{ sidebarOpen: false }">
    ...
    <div class="flex h-full">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 flex flex-col w-64"
            ...
            {{-- Logo --}}
            <div class="flex items-center gap-3 px-6 py-5" style="border-bottom: 1px solid var(--color-surface-border)">
            ...
            <nav class="flex-1 px-3 py-4 overflow-y-auto space-y-0.5">
                ...
                <a href="{{ route('purchase-orders.index') }}"
                    ...
                    Purchase Orders
                    @php
                        $pending = \App\Models\PurchaseOrder::where('shop_id', auth()->user()->shop_id)
                            ->where('status', 'pending')
                            ->count();
                    @endphp
                    @if ($pending > 0 && auth()->user()->isAdmin())
                        <span
                            class="ml-auto badge bg-amber-500/20 text-amber-400 text-[10px]">{{ $pending }}</span>
                    @endif
                </a>
        ...
        <div class="flex-1 flex flex-col min-h-screen lg:pl-64">
            ...
            <header class="sticky top-0 z-40 flex items-center gap-4 px-6 py-3 backdrop-blur-sm"
                ...
                {{-- Branch switcher for admin/owner --}}
                @if (in_array(auth()->user()->role, ['admin', 'owner']))
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" type="button"
                            class="flex items-center gap-2 text-sm bg-slate-800/50 rounded-lg px-3 py-1.5 border border-slate-700 hover:bg-slate-800 transition">
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak
                            class="absolute right-0 mt-2 w-56 rounded-md shadow-lg z-50 border"
                            style="background: var(--color-surface-card); border-color: var(--color-surface-border)">
                            <div class="py-1">
                                @foreach (auth()->user()->shop->branches as $branch)
                                    <form method="POST" action="{{ route('branch.switch') }}">
                                        @csrf
                                        <input type="hidden" name="branch_id" value="{{ $branch->id }}">
                                        <button type="submit"
                                            class="block w-full text-left px-4 py-2 text-sm hover:bg-slate-800 transition-colors
                                   {{ current_branch()?->id === $branch->id ? 'text-white bg-slate-800/50' : 'text-slate-300' }}">
                                            {{ $branch->name }}
                                        </button>
                                    </form>
                                @endforeach
    ...
    <div x-show="sidebarOpen" @click="sidebarOpen = false" x-cloak class="fixed inset-0 z-40 lg:hidden"
