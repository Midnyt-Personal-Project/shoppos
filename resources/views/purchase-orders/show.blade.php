...
@section('content')
    <div class="max-w-5xl space-y-5" x-data="{
        receiveModal: false,
        rejectModal: false,
        activeItem: null,
        qtyReceived: 0,
        ...
        async submitReceive() {
            ...
            if (data.success) { this.receiveModal = false;
                location.reload(); } else alert(data.message);
        },
    }">
        ...
        <div class="flex items-center justify-between flex-wrap gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                @if ($purchaseOrder->isEditable())
                    <a href="{{ route('purchase-orders.edit', $purchaseOrder) }}" class="btn-secondary text-xs">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                {{-- Receive all at once --}}
                @if ($purchaseOrder->isApproved() && $purchaseOrder->status !== 'received' && auth()->user()->isAdmin())
        ...
        <div class="grid md:grid-cols-2 gap-5">
            <div class="card p-5 space-y-2.5">
                ...
                @foreach ([['Reference', '<span class="font-mono text-green-400">' . $purchaseOrder->reference . '</span>'], ['Branch', $purchaseOrder->branch->name], ['Requested By', $purchaseOrder->creator->name], ['Date Created', $purchaseOrder->created_at->format('d M Y, H:i')], ['Expected', $purchaseOrder->expected_at?->format('d M Y') ?? '—'], ['Notes', $purchaseOrder->notes ?: '—']] as [$label, $value])
            ...
            <div class="card p-5 space-y-2.5">
                ...
                @foreach ([['Supplier', $purchaseOrder->supplier_name ?: '—'], ['Phone', $purchaseOrder->supplier_phone ?: '—'], ['Approved By', $purchaseOrder->approver?->name ?? '—'], ['Approved At', $purchaseOrder->approved_at?->format('d M Y, H:i') ?? '—'], ['Items', $purchaseOrder->items->count() . ' products'], ['Est. Total', '₵' . number_format($purchaseOrder->totalCost(), 2)]] as [$label, $value])
                    <div class="flex justify-between items-start">
                        <span class="text-slate-500 text-xs">{{ $label }}</span>
                        <span class="text-slate-300 text-sm text-right">{{ $value }}</span>
                    </div>
                ...
                <tbody class="divide-y divide-slate-800">
                    @foreach ($purchaseOrder->items as $item)
                        <tr class="hover:bg-white/[0.02] transition-colors" @class(['opacity-60' => $item->status === 'received'])>
                            ...
                            <td class="px-3 py-3 text-center text-white font-mono">
                                {{ number_format($item->quantity_requested, 0) }}</td>
                            <td class="px-3 py-3 text-center font-mono">
                                @if ($item->quantity_received > 0)
                                    <span
                                        class="{{ $item->quantity_received >= $item->quantity_requested ? 'text-green-400' : 'text-amber-400' }} font-bold">
        ...
        {{-- Receive Item Modal --}}
        <div x-show="receiveModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
            <div class="card w-[420px] p-6 space-y-4" @click.outside="receiveModal = false">
                ...
                <div class="flex gap-3">
                    ...
                    <button @click="submitReceive()" :disabled="processing" class="btn-primary flex-1"
        ...
        {{-- Reject Modal --}}
        <div x-show="rejectModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
