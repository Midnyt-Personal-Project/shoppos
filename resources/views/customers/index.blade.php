                ...
                <tbody class="divide-y divide-slate-800">
                    @forelse($customers as $customer)
                        <tr class="hover:bg-white/[0.02]">
                            ...
                            <td class="px-3 py-3 text-right text-slate-400">₵{{ number_format($customer->credit_limit, 2) }}
                            </td>
                            <td class="px-3 py-3 text-right">
                                @if ($customer->outstanding_balance > 0)
                                    <span
                                        class="text-red-400 font-bold">₵{{ number_format($customer->outstanding_balance, 2) }}</span>
                                @else
                            ...
                            <td class="px-5 py-3 text-right">
                                <a href="{{ route('customers.show', $customer) }}"
                                    class="text-slate-500 hover:text-white text-xs mr-2">View -></a>
                                <a href="{{ route('customers.edit', $customer) }}"
                                    class="text-blue-400 hover:text-blue-300 text-xs">Edit +</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-12 text-center text-slate-600">No customers yet</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
